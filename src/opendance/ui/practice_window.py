...
import numpy as np
import dataclasses
from PySide6.QtCore import Qt, Slot, QTimer, QUrl, QThread, Signal
...
class PracticeWindow(QWidget):
    def __init__(self, camera_manager: CameraManager, app_config: AppConfig):
        ...
        self._loading_overlay.setStyleSheet("background-color: rgba(0,0,0,210); color: white; font-size: 26px; font-weight: bold;")
    ...
    def _load_video(self):
        path, _ = QFileDialog.getOpenFileName(self, "Select Dance Video", "", "Video Files (*.mp4 *.avi *.mkv)")
        if not path:
            return
            
        self._video_path = path
        self._loading_overlay.setText("Analyzing choreography... Please wait.\n(Extracting features)")
